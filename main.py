...
punchingbagMask = pygame.mask.from_surface(punching_bag) #Mask = where the punchingbag png is
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            player.total_points += upgrade.totalMultiplier()
            ...
            if event.button == 1 and area.collidepoint(pos):
                click_sound.play()
                player.add_score()
            elif event.button == 1 and punchingbagMask.get_at((event.pos[0]- x, event.pos[1]- y)): #If it is at the punching bag png location
                if player.total_points >= 100: #and the points are more than 100 (which is how much u need to buy one)
                    upgrade.punchingBagCount += 1
